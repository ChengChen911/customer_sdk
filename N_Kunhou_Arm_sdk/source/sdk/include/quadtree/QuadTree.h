...
class QuadTree
{
	public:
		...
		void 	draw ();
		std::string 	print ();
		std::vector <std::pair <vertex, T> > getObjectsInRegion (vertex minXY, vertex maxXY);
	...
	private:
		...
		void	reduce (std::stack <QTNode<T>*>& node);
		void 	draw (QTNode<T>* node);
		void 	print (QTNode <T>* node, std::stringstream& ss);
		void	addAllPointsToResults (QTNode<T>* node, std::vector <std::pair <vertex, T> >& results);
