...
public:
	...
	static bool setConfig(const STRING &param_name,
                          const STRING &param_val)
	{
		bool        is_success = false;

		const char *name       = param_name.c_str();
		const char *value      = param_val.c_str();

		try
		{
			BI::managed_shared_memory        cfg_data(BI::open_only, name);
			BI::named_mutex                  cfg_data_mtx(BI::open_only, "cfg_data_mtx");
			BI::scoped_lock<BI::named_mutex> lock(cfg_data_mtx);

			std::pair<BIString*, std::size_t> pair = cfg_data.find<BIString>(name);
			if( 0 != pair.first )
			{
				*pair.first = value;
				is_success = true;
		...
		{
			std::cout<<"*** No such param! ***"<<param_name<<std::endl;
		}
