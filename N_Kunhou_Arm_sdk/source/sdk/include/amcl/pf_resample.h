...
public:
	...
	void update_extra_pos(const SExtra_Pos &extra_pos);
	void update_odom(const SOdomSpeed &odom);
	void stop_scanmatch(const int &st);
	void restart_scanmatch(const int &st);
...
private:
	...
	void reinitialize_extra(SPos extra_pos);
private:// thread
...
private:
	...
	//amcl's A
	THSafe<SExtra_Pos> th_extra_pos_;
	THSafe<int> th_match_stop_;
	boost::mutex mu_odom_;
	SOdomSpeed new_odom_;
	SOdomSpeed last_odom_;
	bool fist_switch_;
	SPos last_amcl_;
	location_tf pos_update_;
	int stop_counts_ ;
	CSBuffer<U32,1> extrapos_buffer_;
public:
