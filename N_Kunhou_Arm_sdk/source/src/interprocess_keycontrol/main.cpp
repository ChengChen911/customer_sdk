...
void call_back_ui( const Srecoder& str ){
	...
	if( "up" == str_key){
		...
		keycode = KEYCODE_D;
	}else if( "goleft" == str_key){
		keycode = KEYCODE_Z;
	}else if( "goright" == str_key){
		keycode = KEYCODE_C;
	}

#endif

	if (cui.set_keycode(keycode,g_speed.vx_,g_speed.vy_,g_speed.vw_))
	{
...
{
	std::cout<<"Navikit Program Version: V1.0.0"<<std::endl;
